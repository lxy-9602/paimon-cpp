...
Decimal ColumnarRowRef::GetDecimal(int32_t pos, int32_t precision, int32_t scale) const {
    ...
    auto array = arrow::internal::checked_cast<const ArrayType*>(ctx_->array_ptrs[pos].get());
    assert(array);
...
std::shared_ptr<InternalRow> ColumnarRowRef::GetRow(int32_t pos, int32_t num_fields) const {
    auto struct_array =
        arrow::internal::checked_cast<const arrow::StructArray*>(ctx_->array_ptrs[pos].get());
    assert(struct_array);
...
std::shared_ptr<InternalArray> ColumnarRowRef::GetArray(int32_t pos) const {
    auto list_array =
        arrow::internal::checked_cast<const arrow::ListArray*>(ctx_->array_ptrs[pos].get());
    assert(list_array);
...
std::shared_ptr<InternalMap> ColumnarRowRef::GetMap(int32_t pos) const {
    auto map_array =
        arrow::internal::checked_cast<const arrow::MapArray*>(ctx_->array_ptrs[pos].get());
    assert(map_array);
